...
const PaymentSuccessPage = () => {
    ...
    const [detailPayment, setDetailPayment] = useState<InvoiceDetails | null>(null);
    ...
    useEffect(()=> {
        ...
        const fetchInvoice = async() => {
            try {
                const resInvoice = await axios.get(`${baseURL}/user/invoices/booking/${bookingID}`, { headers });
                const parseDetails: InvoiceDetails = JSON.parse(resInvoice.data.invoice.details);
    ...
    return (
        ...
        {/* Booking Summary */}
        <div ref={pdfRef} className="p-6 bg-white rounded-lg shadow-md w-full max-w-2xl my-12 border border-primary">
            ...
            <p className="mb-4">By <em className="font-semibold">{ invoice?.seller_name }</em></p>
            ...
            <p className="flex items-center text-gray-700 mb-4">
                <FaPlaneDeparture className="text-primary mr-2" /> { invoice?.departure_location }
            </p>
            <div className="border-t border-black mb-4"></div>
            ...
            <div className="flex flex-col">
                ...
                <h5 className="flex items-center gap-2 font-semibold">
                    ...
                    { handleSumTourist(invoice?.num_adults || 0, invoice?.num_children_120_140 || 0, invoice?.num_children_100_120 || 0)} People
                </h5>
            </div>
            </div>
            ...
            <div className="flex flex-col">
                ...
                <h5 className="text-green-600 uppercase">{ detailPayment?.payment_method}<p className="text-[18px] text-black normal-case">at {formattedDateTime(detailPayment?.payment_date || '')}</p></h5>
            </div>
            <div className="flex flex-col">
                <p className="">Status</p>
                <h5 className="text-green-600">Confirmed</h5>
            </div>
            </div>
            ...
            <h2 className="text-right font-semibold text-primary">{ formatPrice(Number(invoice?.amount_due)) } <span className="text-[18px]">VNĐ</span></h2>
            </div>

            <div className="mt-6 flex justify-center">
                <button onClick={handleDownloadInvoice}  className="bg-primary font-semibold text-white flex items-center px-4 py-2 rounded-lg">
